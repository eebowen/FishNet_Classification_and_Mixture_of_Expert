...
def main() -> None:
    ...
    if args.standardize:
        logger.warning("⚠️  IMPORTANT: Make sure the model was trained WITH --standardize flag!")
    else:
        logger.warning("⚠️  IMPORTANT: Make sure the model was trained WITHOUT --standardize flag!")
    ...
    if args.standardize:
        ...
        # standardize_features returns tuple: (normalized_train, normalized_test, ...)
        normalized = standardize_features(train_features, test_features)
        test_features = normalized[1]  # Get normalized test features
    ...
    try:
        ...
        logger.error("Make sure --hidden-dim matches the trained model")
        raise
